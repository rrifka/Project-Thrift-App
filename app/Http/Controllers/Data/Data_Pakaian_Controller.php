...
class Data_Pakaian_Controller extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $data = [
            ...
            'pakaian_status' => $request->input('status'),
        ];
        Data_Pakaian::createData_Pakaian($data, $gambar);
        Log::info('🟢 Data_Pakaian ' . $request->input('nama') . ' berhasil ditambahkan');
        return redirect()->route('pakaian')->with('success', 'Data pakaian berhasil ditambahkan!');
    }
    public function update (Request $request, $id)
    {
        $ids = Data_Pakaian::find($id);
        if (!$ids) {
            return redirect()->route('pakaian')->with('error', 'Data pakaian tidak ditemukan.');
        }
        $gambar = $request->file('gambar');
        $data = [
            'pakaian_id' => $id,
        ...
        Log::notice('🟡 Data_Pakaian ' . $request->input('nama') . ' berhasil diubah');
        return redirect()->route('pakaian')->with('success', 'Data pakaian berhasil diperbarui!');
    }
    public function delete($id)
    {
        $pakaian = Data_Pakaian::find($id);
        if ($pakaian) {
            Log::alert('🔴 Data_Pakaian dengan ID : ' . $id . ' berhasil dihapus');
            $pakaian->delete();
            return redirect()->route('pakaian')->with('deleted', 'Data pakaian berhasil dihapus!');
        } else {
